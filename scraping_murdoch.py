...
def format_data(index_num: int) -> None:
    print(f"Scraping Episode {index_num}")
    transcript = httpx.request(
        "GET",
        f"https://transcripts.foreverdreaming.org/viewtopic.php?f=13&t={index_num}",
    )
    matches = re.findall(
        r"(?<=<p>).*?(?=<\/p)", transcript.text.replace("<br />\n", " ")
    )
    for match in matches:
        match = re.sub(r"(\[|\()(.*?)(\]|\)|\):)", "", match)
        match = re.sub(r"\b[A-Z]+[A-Z]\b: ", "", match).strip()
        match = (
            match.replace("<script>).push;</script></div>", "")
            .replace("&quot;", '"')
            .replace("<em>", "")
            .replace("</em>", "")
            .replace("<strong>", "")
        ...
        if (
            ...
            and not match.startswith("(")
            and not "&copy;" in match
            and "<" not in match
            and ">" not in match
            and not match.startswith("Announcer: ")
        ):
